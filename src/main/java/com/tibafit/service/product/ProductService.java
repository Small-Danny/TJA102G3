...
public class ProductService {
    ...
    /* 舊版 API（相容） */
    public String sizeOf(String code) { return sizeOf(code, null); }
    ...
    public List<String> sortSizes(Collection<String> sizes) {
        ...
        return sizes.stream()
                ...
                .distinct()
                .sorted(Comparator.comparingDouble(this::sizeWeight)
    ...
    /**
     * 依「同款」群組（keyWithoutSize）挑一個代表 ProductVO。
     * 代表只從「上架」變體中挑；若整組都下架 → 直接不出現在列表。
    ...
    public List<ProductVO> collapseVariants(List<ProductVO> input) {
        ...
        for (List<ProductVO> group : groups.values()) {
            // 只看上架的變體
            List<ProductVO> ups = group.stream()
            ...
            ProductVO rep = ups.stream()
                    .min(Comparator.comparingDouble(p ->
                            sizeWeight(sizeOf(p.getProductCode(), defaultUnitFor(p)))
                    ))
                    .orElse(ups.get(0));
    ...
    private static final Pattern NAME_SIZE_PAREN =
            Pattern.compile("(?iu)[（(]\\s*(?:XS|S|M|L|XL|XXL|\\d+(?:\\.\\d+)?\\s*(?:ML|L|G|KG))\\s*[)）]");
    private static final Pattern NAME_SIZE_LETTER_WITH_HAO =
            Pattern.compile("(?iu)\\b(?:XS|S|M|L|XL|XXL)\\s*(?:號|号)\\b");
    private static final Pattern NAME_SIZE_NUMERIC_UNIT =
            Pattern.compile("(?iu)\\b\\d+(?:\\.\\d+)?\\s*(?:ML|L|G|KG)\\b");
    private static final Pattern NAME_TRAIL_LETTER =
            Pattern.compile("(?iu)(?:-|\\s)+(?:XS|S|M|L|XL|XXL)\\b");
