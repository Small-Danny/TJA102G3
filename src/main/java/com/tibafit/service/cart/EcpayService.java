...
public class EcpayService {
	...
	public EcpayRequest createOrder(OrdersVO newOrder) {
		...
		// ★ 設定綠界成功頁「返回商店」按鈕的目標網址
		// 支援占位符 {orderId}、{orderCode}
		String backUrl = clientBackUrl.replace("{orderId}", String.valueOf(newOrder.getOrderId()))
				.replace("{orderCode}", newOrder.getOrderCode() == null ? "" : newOrder.getOrderCode());
		request.setClientBackURL(backUrl);
	...
	private String generateCheckMacValue(Map<String, String> rawParams) throws Exception {
		...
		Map<String, String> sortedParams = rawParams.entrySet().stream()
				.filter(e -> !"CheckMacValue".equalsIgnoreCase(e.getKey())).sorted(Map.Entry.comparingByKey())
	...
	/**
	 * ECPay 規則的 URL Encode：URLEncoder + toLowerCase + 特定字元還原 ★★★ 關鍵：空白會被轉成
	 * '+'，這點是正確的，不需要換成 %20 ★★★
	 */
	private String ecpayUrlEncode(String s) {
		try {
			String encoded = URLEncoder.encode(s, StandardCharsets.UTF_8.toString()).toLowerCase();
			return encoded.replace("%21", "!").replace("%28", "(").replace("%29", ")").replace("%2a", "*")
					.replace("%7e", "~");
